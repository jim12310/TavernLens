...
public class BuffCounters : Floating {
 ...
 public BuffCounters(Main m){main=m;Text="Blood Gem and Tavern spell bonuses";Size=new Size(288,50);BackColor=Color.FromArgb(1,2,3);TransparencyKey=BackColor;PassThrough(true);}
 public void Follow(Rectangle game){var s=main.State;if(!s.Battlegrounds||!s.Started||s.Complete||s.Phase=="Hero selection"||main.Scenes.InLobby||main.Scenes.Loading){Hide();return;}values=BuffValues.Read(s);string next=s.Key+"|"+values.Gems+"|"+values.Spells;int width=Math.Min(288,game.Width/2);Place(new Rectangle(game.Left+(int)(game.Width*.16),game.Top+(int)(game.Height*.79),width,50));if(next!=signature){signature=next;Invalidate();}}
 protected override void OnPaint(PaintEventArgs e){e.Graphics.Clear(BackColor);e.Graphics.SmoothingMode=SmoothingMode.AntiAlias;using(var image=new Bitmap(410,70)){using(var g=Graphics.FromImage(image)){g.Clear(BackColor);g.SmoothingMode=SmoothingMode.AntiAlias;DrawChip(g,new Rectangle(2,2,197,64),false,values.Gems);DrawChip(g,new Rectangle(207,2,197,64),true,values.Spells);}e.Graphics.InterpolationMode=InterpolationMode.HighQualityBicubic;e.Graphics.DrawImage(image,ClientRectangle);}}
